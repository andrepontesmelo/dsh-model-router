 ...
 *                   cleared by the shim on terminal finish together with the
 *                   failed set). A request that fails outright still
 ...
 *   probe it (`hasCandidate`) without side effects. Retries within one
 ...
 *   `callCtx` (cleared by the shim on terminal finish, together with the
 *   failed set) makes failover RETRIES within the same request re-entrant
 ...
 * - The cursor is per-route state on the algorithm instance; resetting the
 *   shim's per-request failed set does not rewind it.
