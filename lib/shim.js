...
export class RouterShim extends LlmAdapter {
	...
	/** Resolve the route a failover event belongs to (provenance, else last). */
	...
	hasCandidate(signal, provenance) {
		const route = this.routeFor(signal, provenance);
		if (!route) return false;
		return route.algorithmInstance.select(route, this.call(signal, route)) !== void 0;
	}

	markFailed(signal, provenance) {
		const route = this.routeFor(signal, provenance);
		if (!route) return;
	...
	async prepareCall(provider, model, signal) {
		...
		this.requests.get(requestSignal).current = { route, pick };
