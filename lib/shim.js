...
export class RouterShim extends LlmAdapter {
	...
	async *dispatch(provider, model, options, signal, route, pick) {
		...
		yield* this.forward(realCall.stream({ ...options, provider, model, ...this.reasoningOverride(pick, realCall) }), signal, route, pick);
	}

	/**
	 * Per-candidate reasoning override (config `candidates[].reasoning`,
	 * lib/index.js): the configured effort id is checked against the REAL
	 * prepared call's model metadata — the same `reasoning.efforts` shape the
	 * runtime itself validates `reasoningEffort` against — and forwarded as
	 * the dispatched `reasoningEffort` when it names one of them. A model with
	 * no reasoning concept, or without that exact effort, silently keeps the
	 * provider's own default (same skip style as unregistered candidates in
	 * lib/routing.js); the level is adapter-owned, so it can only be judged
	 * where the real model's capabilities are already in hand. Spread order
	 * makes a CONFIGURED level win over anything the incoming options carry;
	 * no configured level returns `{}` and forwards the options untouched.
	 * @returns the options fragment to merge into the forwarded dispatch.
	 */
	reasoningOverride(pick, realCall) {
		if (pick.reasoning === undefined) return {};
		const efforts = realCall.model?.reasoning?.efforts;
		return efforts?.some((effort) => effort.id === pick.reasoning) ? { reasoningEffort: pick.reasoning } : {};
	}
