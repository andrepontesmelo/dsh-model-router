...
const candidate = z.object({
	...
	// Optional reasoning-effort id applied when THIS candidate serves the
	// request (lib/shim.js dispatch). Adapter-owned opaque id — it must name
	// an effort of the real model's advertised efforts; at dispatch time the
	// shim checks the real prepared call's model metadata and IGNORES the
	// level for a model with no reasoning concept or no such effort (same
	// silent-skip style as candidates whose provider is not registered).
	// Plain z.string() is OPTIONAL in schemastery, so candidates without
	// `reasoning` parse identically — backward compatible.
	reasoning: z.string()
});
