...
export function apply(ctx, config) {
	...
	const syncAdapters = () => {
		...
		for (const [provider, group] of nextGroups) {
			...
			const shim = new RouterShim(ctx.llm, routes);
	...
	const listener = ctx.on("agent/request-error", (payload, next) => {
		...
		state.shim.markFailed(payload.signal, payload.provenance);
		return state.shim.hasCandidate(payload.signal, payload.provenance) ? Promise.resolve({ kind: "retry" }) : next();
	});
